...
async function handleGetReceipt({ SN, CEMI }) {
  if (!SN && !CEMI) return { person: '', time: '' }
  const receipts = db.collection('receipts')
  // 1. 优先尝试通过 SN/CEMI 字段直接匹配（兼容可能存在的直存字段）
  const directQuery =
    SN && CEMI
      ? receipts.where({ $or: [{ SN }, { CEMI }] })
      : SN
        ? receipts.where({ SN })
        : receipts.where({ CEMI })
  const directRes = await directQuery.orderBy('createdAt', 'desc').limit(1).get()
  let doc = directRes.data?.[0]
  // 2. 如果直匹配未命中，尝试在 snList 数组中查找包含该 SN 的记录
  if (!doc && SN) {
    const listRes = await receipts.orderBy('createdAt', 'desc').limit(1000).get()
    doc = (listRes.data || []).find(d => {
      const arr = Array.isArray(d.snList) ? d.snList : []
      return arr.includes(String(SN).trim())
    })
  }
  if (!doc) return { person: '', time: '' }
...
exports.main = async (event) => {
  try {
    ...
    if (type === 'get-receipt' || type === 'getReceipt' || type === '领取信息') {
      const data = await handleGetReceipt(body)
      return { code: 0, data }
    }
